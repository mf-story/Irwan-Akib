...
const SCHEMA = [
  ...
  { key: "gallery", title: "Galeri Foto", desc: "Kumpulan foto dokumentasi yang tampil di beranda.", fields: [
    { key: "tag", type: "text", label: "Tag section" },
    { key: "headingHtml", type: "rich", label: "Judul" },
    { key: "sub", type: "textarea", label: "Sub-judul" },
    { key: "items", type: "list", label: "Foto", item: [
      { key: "image", type: "photo", label: "Foto" },
      { key: "caption", type: "text", label: "Keterangan (opsional)" },
    ]},
  ]},
...
function applySideBadge() {
  ...
  else if (CONTENT && CONTENT.brand) badge.textContent = CONTENT.brand.badge || "PD";
  const nm = $("#sideName");
  if (nm && CONTENT && CONTENT.brand && CONTENT.brand.name) nm.textContent = CONTENT.brand.name;
}
function logout() {
